...
def get_nosiy_label():
    ...
    _, train_y,_, test_y = topNochange ()  
...
def get_Domain_label():
    ...
    train_x['y2']=0
    train_x['y3']=0
    train_x.loc[(train_x['max_r_adn']>601.28) & (train_x['max_v_ddn'] > 0)&(test_x['max_i_dup'] <0.2) , "y1"] = 2 
    train_x.loc[(train_x['max_r_adn']>601.28) & (train_x['max_v_ddn'] > 0)&(test_x['max_i_dup'] <0.3) , "y2"] = 2 
    train_x.loc[(train_x['max_r_adn']>601.28) & (train_x['max_v_ddn'] > 0)& (train_x['max_i_dup'] <0.45) , "y3"] = 2 
    
    train_x = train_x[['y1','y2','y3']]
    
    # train_x.to_csv(path+"wk_1_tr4.csv",index = None)
    
    test_x['y1']=0
    test_x['y2']=0
    test_x['y3']=0
    # df.loc[(df['A'] > 2) & (df['B'] == 'c'), 'C'] = 999
    
    test_x.loc[(test_x['max_r_adn']>601.28) & (test_x['max_v_ddn'] > 0) & (test_x['max_i_dup'] <0.2), "y1"] = 2 
    test_x.loc[(test_x['max_r_adn']>601.28) & (test_x['max_v_ddn'] > 0) & (test_x['max_i_dup'] <0.3), "y2"] = 2 
    test_x.loc[(test_x['max_r_adn']>601.28) & (test_x['max_v_ddn'] > 0)& (test_x['max_i_dup'] <0.45) , "y3"] = 2 
   


    test_x = test_x[['y1','y2','y3']]
    
    train_x.to_csv(path+"wk_1_tr5.csv",index = None)   
    
   
    test_x.to_csv(path+"wk_1_test5.csv",index = None)
...
def get_semi_label():
    ...
    df.to_csv(path+"semi_res.csv",index = 0)
    
    # 打包好week lable 数据，此处没有paper中的 test数据进来
    # 需要修改 true label 这一块儿
...
def packfortree():
    ...
    # df.pop("RF")
    # df.pop("GBDT")      
    # X_train,y_train,X_val,y_val = topNochange()
    
    df.rename(columns={'y1':'v_KNN', 'y2':'v_LR','y3':'rof_KNN'}, inplace = True) 
    # df['real'] = y_train
    df.to_csv(path+"test_all.csv",index = None)
    ...
    # df.pop("GBDT") 
    df.rename(columns={'y1':'v_KNN', 'y2':'v_LR','y3':'rof_KNN'}, inplace = True) 
    df.to_csv(path+"tr_all.csv",index = None)
