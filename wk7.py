...
def LF_2(df):  
    ...
    # df.loc[(df.max_v_dup/ df.max_v_ddn>2.5)| (df.max_v_ddn/ df.max_v_dup>2.5),"LF3" ] = 1
    return df
...
def get_semi_label():
    ...
    from sklearn import tree  
    base_classifier = tree.DecisionTreeClassifier(max_depth = 2)    
...
def check_static():
    ...
    print(df1[df1["label"]==0].shape)
    print(df1[df1["label"]==1].shape)
    print(df1[df1["label"]==2].shape)
    print(df1[df1["label"]==3].shape)
    print(df1.shape)
