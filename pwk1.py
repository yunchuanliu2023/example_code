...
def top(rate,ii):    
    ...
    df2 = pd.read_csv("random/unlabeled_"+str(ii)+"_"+str(rate)+"%.csv")
...
def filter_data(rate,i):
    ...
    print(df2.shape[0],df4.shape[0],df1.shape[0],df5.shape[0])
    print(r1,r2,r3)
...
def get_nosiy_label2(rate,ii):
    # data_file = "H:\\Research\\data\\trainCG.csv"  
    s1 = timeit.default_timer()  

    train_x, train_y,test_x, test_y =top(rate,ii)
...
def packit():
    ...
    df.to_csv(path+"test_all2.csv",index = None)
        
    df = pd.read_csv(path+"wk_1_tr2.csv")
    for i in range(3,5):
        df2 = pd.read_csv(path+"wk_1_tr"+str(i)+".csv")
        df  = pd.concat([df, df2], axis=1)
    df.to_csv(path+"tr_all2.csv",index = None)
...
def gen(rate,ii):
    ...
    get_nosiy_label2(rate,ii)
    get_nosiy_label3(rate,ii)
    get_nosiy_label4(rate,ii)
    # get_semi_label()
    snoit()
